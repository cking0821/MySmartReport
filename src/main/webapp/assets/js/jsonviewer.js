...
function setToEditor(data) {
	...
	if (data.trim().length > 0) {
		if(viewname=='jsonviewer' || viewname=='jsonminifier'){
			editorResult.getSession().setMode("ace/mode/json");
		}
		try {
...
function validateJSON() {
	...
	if (data != null && data != "" && data.trim().length > 0) {
		...
		try {
			...
			jsonValidateError("JSON String is valid but JSON Data is not accurate");
		}
		setOutputMsg("Validate Json");
	}
}

function jsonviewerTocsv() {

	var csv = jsonTocsvbyjson(editorAce.getValue(),true);
